...
Migration `005_invoice_last_checkout_attempt_at.sql` adds a nullable `TIMESTAMPTZ` column to `invoices`. The checkout handler updates it after build-XDR or submit-XDR attempts.
...
- No standalone index is created on `invoices.last_checkout_attempt_at`. Detailed attempt history is indexed in `checkout_attempts` by migration `021_checkout_attempts.sql`.
...
**Verify:** `cargo test checkout_attempt` and `npm test -- checkoutAttempts` (guard the migration shape and checkout-attempt audit writes).
